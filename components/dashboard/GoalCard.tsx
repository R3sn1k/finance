"use client";

type Props = {
  year: number;
  dobiček: number;
  letniCiljDobicka: number;
  fmtDob: string;
  fmtCilj: string;
  fmtPreostanek: string;
  fmtIzguba: string;
  odstotekDobicka: number;
};

export default function GoalCard({
  year,
  dobiček,
  fmtDob,
  fmtCilj,
  fmtPreostanek,
  fmtIzguba,
  odstotekDobicka,
}: Props) {
  return (
    <div className="glass-panel rounded-2xl p-6 mb-8">
      {/* Title + current vs goal */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-4">
        <h3 className="text-lg sm:text-xl font-bold text-white">Letni cilj dobička {year}</h3>
        <span className="text-xl sm:text-2xl font-black text-white">
          {fmtDob} € <span className="text-sm text-slate-400 font-normal">/ {fmtCilj} €</span>
        </span>
      </div>

      {/* Progress bar */}
      <div className="relative h-12 bg-slate-950/80 rounded-full overflow-hidden mb-4 ring-1 ring-white/10">
        <div
          className={`absolute inset-0 h-full transition-all duration-1000 ease-out ${
            dobiček < 0
              ? "bg-gradient-to-r from-red-500 to-rose-600"
              : odstotekDobicka >= 100
              ? "bg-gradient-to-r from-emerald-500 to-green-600"
              : "bg-gradient-to-r from-teal-400 to-blue-600"
          }`}
          style={{ width: `${dobiček < 0 ? 100 : Math.min(odstotekDobicka, 100)}%` }}
        >
          <div className="absolute inset-0 bg-white opacity-20 animate-pulse"></div>
        </div>

        <div className="absolute inset-0 flex items-center justify-center">
          <span className="text-xl sm:text-2xl font-black text-white drop-shadow-md">
            {dobiček < 0 ? "IZGUBA" : `${odstotekDobicka.toFixed(0)}%`}
          </span>
        </div>
      </div>

      {/* Status text */}
      <div className="text-center">
        {dobiček < 0 ? (
          <p className="text-base sm:text-lg font-bold text-red-300">V izgubi za {fmtIzguba} €</p>
        ) : odstotekDobicka >= 100 ? (
          <p className="text-lg sm:text-xl font-bold text-emerald-300">ČESTITKE! CILJ PRESEŽEN!</p>
        ) : (
          <p className="text-base sm:text-lg text-slate-300">
            Še <span className="font-bold">{fmtPreostanek} €</span> do cilja
          </p>
        )}
      </div>
    </div>
  );
}
